...
func (exp *ExportController)exportTaxLink(c *gin.Context){
	log.Println("ExportController exportTaxLink start")
	var header crvClient.CommonHeader
	if err := c.ShouldBindHeader(&header); err != nil {
		log.Println(err)
		rsp:=common.CreateResponse(common.CreateError(common.ResultWrongRequest,nil),nil)
	...
	rsp:=exportTaxLink(&rep,&header,exp.CRVClient,c)
	//加载一个流的配置
	if rsp!=nil {
		c.IndentedJSON(http.StatusOK, rsp)
	}
	log.Println("ExportController exportTaxLink end")
}

func (exp *ExportController) Bind(router *gin.Engine) {
	log.Println("Bind FlowController")
	router.POST("/export/cdn", exp.exportCDN)
	router.POST("/export/taxlink", exp.exportTaxLink)
}